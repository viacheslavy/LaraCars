...
class BlogSpider(scrapy.Spider):
	name = 'hemmings'
	def __init__(self,start_year=None,end_year=None,start_price=None,end_price=None,page_number=None,brand=None,model=None,model_group_facet=None,model_facet=None, *args, **kwargs):
		...
		url= 'https://www.hemmings.com/classifieds/cars-for-sale/?year_min='+start_year+'&year_max='+end_year+'&price_min='+start_price+'&price_max='+end_price+'&page_size='+str(page_size)+'&start='+str(start)+'&makeFacet='+str(brand)+'&model_groupFacet='+str(model_group_facet)
		if model_facet:
			model_facet = model_facet.split(',')
			for model_fac in model_facet:
				url=url+'&modelFacet[]='+str(model_fac)
		self.start_urls = [url]
	...
	def product_details(self, response):
		...
		for re in response.css(".royalSlider.rsDefaultHMN .rsImg"):
			...
			img = img.replace("100-0@2X.jpg", "770-0@2X.jpg")
			src.append(img)
		...
		if "Price:" in specification:
			price_pos=specification.index("Price:")
		if seller_description:
			if price_pos:
				des= specification[seller_description+1:price_pos]
			else:
				des= specification[seller_description+1:len(specification)]
		...
		yield {"href":response.url,"title":title,"price":price,"src":src,"specs":specs,"seller_name":seller_name,"year":year,"brand":brand,"model":model,"reference_id":reference_id,"kilometers":kilometers,"mileage":mileage,"page_count":total_pages,"total_results":total_results}